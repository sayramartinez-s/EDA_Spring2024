...
#Print the plot
print(Plot_text)
# Another example is using the symbol of degrees, instead of mu: label=expression(paste("Mean is ", mu,",", R^{2},'=0.6'))
...
###If I would want to keep just those with frequencies, I would change drop for T
...
#theme_classic (this is other option of team-it's plain white)
...
ggplot(PeterPaul.chem.nutrients, aes(x = daynum, y = temperature_C, color=year4)) +
  geom_point()+labs(x="Day of sampling", y="Temperature C")+
  facet_wrap(PeterPaul.chem.nutrients$lakename)+ labs(color="Year") + theme_wsj()
...
ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C, color=depth)) +
  geom_point()+labs(x="Date of sampling", y="Temperature C")+
  facet_wrap(PeterPaul.chem.nutrients$lakename)+ labs(color="Depth") + theme_wsj()
