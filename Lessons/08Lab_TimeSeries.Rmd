...
avg_rainfall
ggplot(BR_complete) +
  geom_line(aes(x = Date, y = Rainfall), color = "darkred") +
...
avg_temp <-
ggplot(BR_complete) +
  geom_line(aes(x = Date, y = Temperature_C), color = "royalblue") +
...
BR_rain_ts <- ts(BR_complete$Rainfall, start = c(1901,1),
                 frequency = 12)

BR_temp_ts <- ts(BR_complete$Temperature_C, start = c(1901,1),
                 frequency = 12)
...
Decomposed_BR_rain <- stl(BR_rain_ts, s.window = "periodic")
plot(Decomposed_BR_rain)

Decomposed_BR_temp <- stl(BR_temp_ts, s.window = "periodic")
plot(Decomposed_BR_temp)
...
df.decomposed.BR.Rain <- as.data.frame(Decomposed_BR_rain$time.series[,1:3])

df.decomposed.BR.Rain <- mutate(df.decomposed.BR.Rain,
        Observed = BR_complete$Rainfall,     
        Date = BR_complete$Date)


df.decomposed.BR.temp <- as.data.frame(Decomposed_BR_temp$time.series[,1:3])

df.decomposed.BR.temp <- mutate(df.decomposed.BR.temp,
        Observed = BR_complete$Temperature_C,     
        Date = BR_complete$Date)

# Exercise 10: Visualize how the trend maps onto the data for both series

ggplot(df.decomposed.BR.temp) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff")
  

ggplot(df.decomposed.BR.Rain) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff")
...
BR_rain_trend <- Kendall::SeasonalMannKendall(BR_rain_ts)
summary(BR_rain_trend)

BR_temp_trend <- Kendall::SeasonalMannKendall(BR_temp_ts)
summary(BR_temp_trend)

rain_trend1 <- trend::smk.test(BR_rain_ts)
summary(rain_trend1)

temp_trend2 <- trend::smk.test(BR_temp_ts)
summary(temp_trend2)
