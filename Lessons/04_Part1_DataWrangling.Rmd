...
`>`
`>=`
`&` #conditional, both are true/false
...
NTL.phys.data.1984y <- filter(NTL.phys.data, year4 == 1984)
# Exercise: 
...
NTL.phys.data.descendingtemp <- arrange(NTL.phys.data, desc(temperature_C))
head(NTL.phys.data.descendingtemp)
```
### Select
...
NTL.phys.data.hightemps <- arrange(NTL.phys.data.temps, desc(temperature_C))
head(NTL.phys.data.hightemps)
